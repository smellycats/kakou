...
<body>
	...
	<?php $lgquery = $this->session->userdata('lgquery');?>
	var ids = <?php echo $ids;?>;
	var id = <?php echo $id;?>;
	var rows = <?php echo $lgquery['rows'];?>;
	var total = <?php echo $lgquery['total'];?>;
	var offset = <?php echo $lgquery['offset']?>;
    ...
    function next(){
        ...
        if (next+offset >= total){
            ...
            $.post('<?php echo base_url() . "index.php/logo/get_lg_ids"."'";?>, { offset: offset+rows, rows:rows}, function (result) {
                    if (result.success) {
                    	ids = result.ids;
                    	offset = offset+rows;
                    	id = ids[0];
    ...
    function goUrl(x)
    {
		...
		window.location.replace(<?php echo "'".base_url() ."index.php/logo/lgquery"."'";?>);
    }
