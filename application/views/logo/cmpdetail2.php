...
function loadDataP(){
	window.location.href=<?php echo '"' . base_url() .  'index.php/logo/cmpdetail/'. ($current_id-1) . '"'?>;
}
...
function comeback(){
	window.location.href=<?php echo '"' . base_url() .  'index.php/logo/cmpquery_ok/' .  '"'?>;
}
--></script>
	...
	<div class="clearfix" style="border:0px;float:left;width:100%;">
		...
		<div style="float:left; width:900px; border-top:1px solid #eee;border-left:1px solid #eee; border-right: 1px solid #888; border-bottom: 1px solid #888;background-color:#fafafa;">
			...
			<div style="float:left;width:300px; ">
				...
				<form method="post" action="<?php echo site_url('logo/cmpdetail_ok'); ?>">
					<table align="center" border="0" cellspacing="0" cellpadding="6" style="padding: 0px 0px;" >
						<tr>
							<td><input type="checkbox" name="clppflag" id="clppflag" value="1" <?php if($carinfo->clppflag==1){echo 'checked = "checked"';}?>/><span style="color:blue">识别信息与图片车辆类型不匹配</span></td>
						</tr>
						<tr>
							<td><input type="checkbox" name="smsflag" id="smsflag" value="1" <?php if($carinfo->smsflag==1){echo 'checked = "checked" disabled="disabled"';}?>/><span style="color:blue">发送警报短信</span></td>
						</tr>
						<tr>
							<td>选择发送号码:</td>
							<td>
				                <select name="sms">
				                <?php foreach ($sms as $id=>$row): ?>
				                <option value="<?php echo $row['id']; ?>" <?php if ($this->session->userdata('sms_id')==$row['id']){echo " selected";} ?>><?php echo '#'.$id.' : '.$row['mark']; ?></option>
				                <?php endforeach; ?>
				                </select>
							</td>
						</tr>
						<tr>
							<td>
								<input type="submit" name="submit" value="<?php if($carinfo->confirmflag==1){echo '重新确认';}else{echo '提交';}?>" />
							</td>
						</tr>
					</table>
				</form>
