...
<body>
		...
		<div id="tb" style="padding:3px 20px">
			<div align="center">
				...
				<input id="carsize" class="easyui-combobox" style="width:60px"
						...
						method = "get" valueField = "id" textField = "text" />
				<span>报警类型:</span>
				<input id="alarmtype" class="easyui-combobox" style="width:80px"
	...
	<script type="text/javascript">
		...
		function doSearch(){ 
            $.post('<?php echo base_url() . "index.php/stat/stat_data"."'";?>, { place: $('#place').combobox('getValue'),st:$('#st').combobox('getValue'),et:$('#et').combobox('getValue'),carsize:$('#carsize').combobox('getValue'),alarmtype:$('#alarmtype').combobox('getValue')}, function (result) {
                    title = result.title;
                    data = result.data
                    for(var i = 0; i < data.length; i++) {
                    	total += data[i];
