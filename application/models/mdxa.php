...
class Mdxa extends CI_Model
{
	...
	{
		return $this->ora_db->query("select * from WLCP where memo='待销案车辆'");
	}
	
	//根据车牌查询白名单信息
	function get_whilelist_by_num($carnum)
	{
		return $this->ora_db->query("select * from WLCP where HPHM='$carnum' and memo='待销案车辆'");
	}
	...
	function add_wl($data)
	{
		...
		return $this->ora_db->query("Insert Into WLCP(HPHM,HPYS,CLBJ,MEMO)Values('$data[carnum]', '$data[color]', 'F', '待销案车辆')");
	}
	
	//根据车牌号查找白名单内容
	function find_wl_by_carnum($data)
	{
        return $this->ora_db->query("select * from WLCP where hphm like '$data[carnum]' and  memo='待销案车辆' ORDER BY HPHM DESC");
	}
	...
	function get_dxa_by_condition($offset = 0, $row_count = 0, $data)
	{
		...
		if ($offset >= 0 AND $row_count > 0)
	    {
			...
			$query = $this->ora_db->query("SELECT t.*, jgsj as passtime FROM (SELECT A.*, ROWNUM RN FROM (SELECT a.* FROM wcltx a, WLCP b where a.HPHM = b.HPHM and b.memo='待销案车辆' " . $sqlstr . " order by a.id desc) A WHERE ROWNUM <= $max_count)t WHERE RN > $offset" );
		}
			else
		{
			//返回查询结果行数
			$query = $this->ora_db->query("select count(1) as sum  from wcltx a, WLCP b where a.HPHM = b.HPHM and b.memo='待销案车辆' " . $sqlstr);
		}
