...
class Userlog extends Admin_Controller
{
	...
	function view()
	{
		...
		$data['result'] = $this->Muserlog->getUsers($data['offset'], $data['rows'], $data['sort'], $data['order'], $data)->result_array();
		$data['total']  = $this->Muserlog->getUsers(0, 0, $data['sort'], $data['order'], $data)->row()->sum;
