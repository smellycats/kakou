...
class Wmenu extends Admin_Controller
{
	//车道
	...
	function wlset()
	{
		...
		//$data[]
		$data['whilelist'] = $this->Mwmenu->get_whitelist_all()->result();
		
		$this->load->view('wmenu/wlset', $data);
	}
	...
	function edit_wl_check()
	{
		...
		$this->Mwmenu->edit_whilelist($id, $data);
	...
	function add_wl_ok()
	{
		...
		else
		{
		    ...
		    $res = $this->Mwmenu->add_wl($data);
		    
			if ($res)
			{
				showmessage('添加白名单成功', 'wmenu/wlset');
			}
			else
			{
				showmessage('操作失败，系统繁忙或着填写错误', 'wmenu/wlset');
			}
		}
	}
	...
	function find_wl()
	{
		...
		else
		{
			...
			$data['whilelist'] = $this->Mwmenu->find_wl_by_carnum($data)->result();
	...
	function wquery()
	{
		...
		$config['base_url'] = site_url('wmenu/wquery/');
		...
		$this->load->view('wmenu/wquery', $data);
	}
    ...
    function wquery_ok()
    {
		...
		$row_count = 20;
		// Get all users
		$data['car'] = $this->Mwmenu->get_wmenu_by_condition($offset, $row_count, $da)->result();
		$data['total_rows']  = $this->Mwmenu->get_wmenu_by_condition(0,0,$da)->row()->SUM;
		$data['total_pages'] = intval($data['total_rows']/$row_count)+1;
		$data['offset']      = $offset;
			
		$config['base_url'] = site_url() . "/wmenu/wquery_ok/?type_alias=$data[sel_type_alias]&lane=$da[lane]&direction=$da[direction]&color=$da[color]&number=$da[number]&carnum=$da[carnum]&starttime=$da[starttime]&endtime=$da[endtime]";
		$config['total_rows'] = $this->Mwmenu->get_wmenu_by_condition(0,0,$da)->row()->SUM;
		$config['per_page'] = $row_count;
    ...
    function del_wl_set()
    {
    	...
    	$res = $this->Mwmenu->del_whilelist_by_id($id);
    	
    	if ($res)
		{
			showmessage('删除白名单车辆信息成功', 'wmenu/wlset');
		}
		else
		{
			showmessage('操作失败，系统繁忙或着填写错误', 'wmenu/wlset');
	...
	function wl_showdetail()
	{
		...
		$data['color']     = $this->color;    
		$data['cardetail'] = $this->Mwmenu->get_wmenu_by_id($id)->row();
		$data['pic_url']   = get_picurl($data['cardetail']->QMTP, $data['cardetail']->TJTP);
		
		$this->load->view('wmenu/wl_showdetail', $data);
	}
