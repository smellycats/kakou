...
class Oplog extends Admin_Controller
{
	...
	function view()
	{
		...
		$data['result'] = $this->Muserlog->getUserlogs($data['offset'], $data['rows'], $data['sort'], $data['order'], $data)->result_array();
		$data['total']  = $this->Muserlog->getUserlogs(0, 0, $data['sort'], $data['order'], $data)->row()->sum;
